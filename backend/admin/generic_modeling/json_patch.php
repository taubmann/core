...
class JsonPatch
{
  ...
  // chris: fix the Sort-Order on the first Level
  public function fixOrder($dst, $new)
  {
	  $keys = array_keys($dst);
	  $ret = array();
	  foreach($keys as $key) $ret[$key] = $new[$key];
	  return $ret;
  }
...
/*  <fizz>wizz</fizz> -> "wizz" */
  ...
  private static function diff_array($path, $src, $dst)
  {
    ...
    {
      if ($i < $lsrc && $i < $ldst)// && isset($src[$i]) && isset($dst[$i])
      {
        $result = array_merge($result,
