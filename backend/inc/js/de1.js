// AUTO-CREATED FILE (created at 1356799746) do not edit!
...
function createContent() {
	...
	function(data) 
	{
		...
		if(data.substr(0,1)=='_') 
		{
			...
			getContent(data);
			message('Eintrag erzeugt'+' (ID:'+data+')');
		}
	});
};
...
{
	var q = confirm('Eintrag löschen'+' (ID:'+id+')?');
	if(q) {
		$.get('crud.php', 
		{ 
		...
		function(data) 
		{
			...
			$('.sublistParent').bind('sortreceive', function(event, ui) 
			{
				if ($(this).children().length > 2) 
				{
					...
					message('Aktion abgebrochen, da nur ein Element erlaubt ist', true, 5000);
					return;
				};
			});
			...
			$('#referenceSearchbox').autocomplete(
			{
				...
				select: function(event, ui)
				{
					...
					var htm = '<h4>'+'was möchten Sie tun?'+'</h4><button type="button" onclick="'+lnk+'">'+'Eintrag zeigen'+'</button> ';
					...
					{
						htm += '<button type="button" id="insertListItem">'+'Eintrag verknüpfen'+'</button>';
					};
