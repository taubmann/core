<?php
/**
 * XOR encrytion algorithm
 * this algorithm is way faster than Blowfish
 * 
 * */
class X_OR {
	/**
	 * use this to encrypt short Strings via XOR
	 * @param plaintext	string	String/File to encode
	 * @param key	string	secret Key
	 * */
	function encrypt($plaintext, $key)
	{
		$o = '';
		$c = base64_encode($plaintext);
		for($i=0; $i<strlen($c);)
		{
	...
	/**
	 * use this to decrypt short Strings via XOR
	 * @param ciphertext	string	String/File to decode
	 * @param key	string	secret Key
	 * */
 ...
 * 
 * 
 * Copyright © 2009 Matt Harris (http://themattharris.com) under MIT
 * @author Matt Harris
 * 
 * adapted: https://github.com/halfdan/PHP-Blowfish
 **/
...
class Blowfish {
   ...
   * @author Matt Harris
   **/
  public function encrypt($plaintext, $key, $iv=null, $mode=Blowfish::BLOWFISH_MODE_CBC, $padding=Blowfish::BLOWFISH_PADDING_RFC)
  {
   ...
   * 
   **/
  public function decrypt($ciphertext, $key, $iv=null, $mode=Blowfish::BLOWFISH_MODE_CBC, $padding=Blowfish::BLOWFISH_PADDING_RFC)
  {
    if ( $mode == Blowfish::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
